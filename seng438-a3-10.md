...
Below are figures showing the three coverages that we tested. Our branch and statement coverages ended up being lower at certain points due to the inability to test many of the functions if they were to have invalid inputs, which often caused some of the branches to not be covered, this included not being able to create a range with a lower bound that had a greater value than the upper bound, which caused any if statements that included *lower<upper* to only cover one branch.
